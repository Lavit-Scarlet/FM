...
{
    public class SpectraliburSlash : SlashSpectralibur
    {
        ...
        public override void AI()
        {
            if (AttackAI[1] == 1f)
            {
                for (int i = 0; i < 8; i++)
                {
                    int a = Dust.NewDust(Projectile.position, Projectile.width, Projectile.height, DustID.DungeonSpirit);
        ...
        {
            FMHelper.PlaySound(new SoundStyle($"{nameof(FM)}/Assets/Sounds/BigSlash"), (int)Projectile.Center.X, (int)Projectile.Center.Y, .50f);
            //SoundEngine.PlaySound(SoundID.Item1, Projectile.Center);
            base.AttackPro();
        }
    }
    public abstract class SlashSpectralibur : ModProjectile
    {
