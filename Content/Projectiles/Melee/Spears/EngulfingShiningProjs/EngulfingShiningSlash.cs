...
{
    public class EngulfingShiningSlash : EngulfingShiningSlashSlash
    {
        ...
        {
            float Dis = (Projectile.width + Projectile.height + AttackAI[0]) * 0.48f;
        ...
        {
            float Dis = (Projectile.width + Projectile.height + AttackAI[0]) * 0.48f;
            Vector2 B = Pos + Vec * Dis * Projectile.scale * 0.35f;
            return B;
        }
        public override bool PreDraw(ref Color lightColor)
        {
            if (AttackAI[1] == 1f)
            {
                Color[] LColor = new Color[] { Color.DarkViolet };
                AttackEffects(LColor);
                AttackEffects(new Color(255, 255, 255));
            }
            return false;
        }
        ...
        {
            FMHelper.PlaySound(new SoundStyle($"{nameof(FM)}/Assets/Sounds/BigSlash"), (int)Projectile.Center.X, (int)Projectile.Center.Y, .50f, 0.6f);
            base.AttackPro();
        }
		public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
            FMHelper.PlaySound(new SoundStyle($"{nameof(FM)}/Assets/Sounds/Slashing"), (int)Projectile.Center.X, (int)Projectile.Center.Y, .35f, .35f);
    ...
    public class EngulfingShiningSlash3 : EngulfingShiningSlashSlash
    {
        public override string Texture => "FM/Content/Projectiles/Melee/Spears/EngulfingShiningProjs/EngulfingShiningSlash";
        public override void SetStaticDefaults()
        {
            ProjectileID.Sets.TrailingMode[Projectile.type] = 2;
        ...
        public override Vector2 PosB(Vector2 Pos, Vector2 Vec)
        {
            Vector2 B = Pos + Vec * AttackAI[0] * Projectile.scale * 1.35f;
            return B;
        }
        public override bool PreDraw(ref Color lightColor)
        {
        ...
        public override void AttackPro()
        {
            FMHelper.PlaySound(new SoundStyle($"{nameof(FM)}/Assets/Sounds/BigSlash"), (int)Projectile.Center.X, (int)Projectile.Center.Y, .50f, 0.2f);
            base.AttackPro();
        }
    }
    public abstract class EngulfingShiningSlashSlash : ModProjectile
    {
