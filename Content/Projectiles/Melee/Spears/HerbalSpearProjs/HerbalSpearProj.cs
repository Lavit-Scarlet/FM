...
{
	public class HerbalSpearProj : ModProjectile
	{
		...
		public override bool PreAI()
		{
			...
			if(degrees > 55 * nextProj && nextProj <= 3)
            {
				nextProj++;
				if (Main.myPlayer == Projectile.owner)
				{
					Vector2 perturbedSpeed = Projectile.velocity;
					Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center, perturbedSpeed, ModContent.ProjectileType<HerbalSpearShootProj>(), Projectile.damage, Projectile.knockBack, Projectile.owner, 0, -2);
				}
			}
			return false;
		}
