...
{
    public class WorldStaffProj : ModProjectile
    {
        private int Counter
		{
        ...
        private void DrawSecondaryBeam()
		{
			...
			float initScaleY = 200;

			for (int i = 0; i < 3; i++)
			{
				Texture2D texture = Mod.Assets.Request<Texture2D>("Assets/Textures/Trails/Lightning").Value;
        ...
        private void CheckCollision()
        {
			...
			FMHelper.PlaySound(SoundID.Item122, origin + (dirUnit * shotLength), 1f);

			for (int i = 0; i < 8; i++)
			{
				Dust dust = Dust.NewDustPerfect(origin + (dirUnit * shotLength), 226, Vector2.Zero, 0, Color.White, Main.rand.NextFloat(1.0f, 1.5f));
				dust.velocity = -(Projectile.velocity * Main.rand.NextFloat(0.2f, 0.9f)).RotatedByRandom(0.8f);
				dust.noGravity = true;
                dust.scale = 1f;
			}
			for (int i = 0; i < 1; i++)
			{
				ParticleManager.NewParticle<HollowCircle_Small>(origin + (dirUnit * shotLength), Projectile.velocity, Color.White, 0.72f, 0, 0);
			}
			for (int i = 0; i < 5; i++)
			{
				ParticleManager.NewParticle<LineStreak_Long_ShortTime>(origin + (dirUnit * shotLength), new Vector2(Main.rand.NextFloat(-3f, 3f), Main.rand.NextFloat(-3f, 3f)), Color.White, Main.rand.NextFloat(0.5f, 1.4f), 0, 0);
			}
        }
