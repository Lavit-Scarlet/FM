...
{
	public class GrimoireofAnotherDimensionLaserProj : ModProjectile
	{
		...
		public override void AI()
		{
			if (Projectile.localAI[1] == 0f)
			{
				...
				FMHelper.PlaySound(new SoundStyle($"{nameof(FM)}/Assets/Sounds/BeamSwing"), (int)Projectile.Center.X, (int)Projectile.Center.Y, .40f, .0f);
			}
			...
			else if (State == 1)
			{
				...
				if (time >= 4)
				{
					if (Projectile.owner == Main.myPlayer)
					{
						...
						Projectile.NewProjectile(Projectile.GetSource_FromAI(), Projectile.Center, Projectile.velocity, ModContent.ProjectileType<GrimoireofAnotherDimensionShootLaserProj>(), Projectile.damage, Projectile.knockBack, Main.myPlayer, Projectile.whoAmI);
					}
					State = 2;
				}
			}
