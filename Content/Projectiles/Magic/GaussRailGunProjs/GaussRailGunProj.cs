...
{
    public class GaussRailGunProj : ModProjectile
    {
        private int Counter
		{
        ...
        private void CheckCollision()
        {
            ...
            if (target != null)
			{
				...
				{
					float positionX = Main.rand.Next(-60, 60);
					float positionY = Main.rand.Next(-40, -0);
					int a = Projectile.NewProjectile(Projectile.InheritSource(Projectile), player.Center.X + positionX, player.Center.Y + positionY, 0f, 0f, ProjectileType<GaussRailGunTwoProj>(), Projectile.damage, 2f, player.whoAmI);
					Vector2 vector2_2 = Vector2.Normalize(new Vector2(target.Center.X, target.Center.Y) - Main.projectile[a].Center) * (float)Main.rand.Next(5, 5);
			...
			for (int i = 0; i < 12; i++)
			{
				...
				dust.velocity = -(Projectile.velocity * Main.rand.NextFloat(0.4f, 0.8f)).RotatedByRandom(0.9f);
				dust.noGravity = true;
                dust.scale = 1f;
			}
        }
		...
		//public override void OnHitPvp(Player target, int damage, bool crit) => SpawnProj();
    }
}