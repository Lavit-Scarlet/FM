...
namespace FM
{
	public sealed class ShaderLoader : ILoadable
	{
		...
		public void Load()
		{
			...
			var file = (TmodFile)info.Invoke(FM.Instance, null);
		...
		{
			var shaderRef = new Ref<Effect>(FM.Instance.Assets.Request<Effect>(shaderPath, AssetRequestMode.ImmediateLoad).Value);

			(Filters.Scene[FM.Abbreviation + ":" + shaderName] = new Filter(new ScreenShaderData(shaderRef, shaderName), EffectPriority.High))
				.Load();
		}
	}
}
